    ...
    searchDetailAddrFromCoords(mouseEvent.latLng, function (result, status) {
      if (status === kakao.maps.services.Status.OK) {
        var detailAddr = !!result[0].road_address
          ...
          : "";
        detailAddr +=
          "<div>지번 주소 : " + result[0].address.address_name + "</div><div>이 곳을 이벤트 등록 위치로 설정합니다.</div>";
        ...
        // 지도에 찍은 위치를 location_form에다가 넣어주는 작업
        let city_input = document.querySelector('#id_city');
        let gu_input = document.querySelector('#id_gu');
        let rest_address_input = document.querySelector('#id_rest_address');

        city_input.value = cityName
  ...
  function map_reset(e) {
    ...
    let choosed_city = siblings.filter(
      (node) => node.className == "choose_city"
    );
